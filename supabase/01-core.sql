...
-- (Dashboard Ú∆“ SQL Editor Ú∆“ New Query Ú∆“ Paste Ú∆“ Run)
...
-- Uses auth.uid() for security Ú¿‘ only the logged-in user can claim for themselves
...
-- The function source is NOT readable by anon users Ú¿‘ only database admins can see it
...
--    In Supabase Dashboard Ú∆“ Edge Functions Ú∆“ Secrets:
...
--    Option B (Dashboard): Supabase Ú∆“ Edge Functions Ú∆“ New Function
--      Ú∆“ Name: telegram-auth Ú∆“ Paste code from supabase/functions/telegram-auth/index.ts
--
-- 5. UPDATE js/config.js:
--    Set window.TELEGRAM_BOT_USERNAME = 'your_bot_username'  (without @)
--
-- 6. KEEP EMAIL AUTH ENABLED:
--    Do NOT disable email auth in Supabase Dashboard Ú¿‘ the admin panel
...
-- (Drop and recreate if it exists ¶-¶¬Ú¿› the function may vary, so this is additive)
...
BEGIN
    ...
    SELECT
        ...
        (SELECT COALESCE(vp.telegram_username, vp.telegram_first_name, '¶–¶+¶-¶¨¶-')
         FROM profiles vp
...
BEGIN
    ...
    IF v_old_invite_id IS NOT NULL THEN
        IF v_old_invite_used THEN
            RAISE EXCEPTION '¶·T¬¶-T¿TÀ¶¶ ¶¨¶-¶-¶-¶¶T¬ T√¶¶¶¶ ¶¨T¡¶¨¶-¶¨TÃ¶¨¶-¶-¶-¶- Ú¿‘ ¶¨¶¶T¿¶¶¶¶¶¶¶-¶¶T¿¶-T∆¶¨Tœ ¶¨¶-¶¨T¿¶¶T…¶¶¶-¶-';
        END IF;
